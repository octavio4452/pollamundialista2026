    ...
    <div className="space-y-6">
      {BRACKET_CATS.map((cat, idx) => {
        ...
        const prevSelectedIds = prevCat
          ? preds.filter((p: any) => p.category === prevCat.key).map((p: any) => p.team_id)
          : null;
        const eligibleTeams = prevSelectedIds
          ? teams.filter((t: any) => prevSelectedIds.includes(t.id))
          : teams;
        const visibleTeams = locked
          ? teams.filter((t: any) => selected.some((s: any) => s.team_id === t.id))
          : eligibleTeams;
        const needsPrev = prevCat && prevSelectedIds!.length < prevCat.count;
        return (
          <Card key={cat.key} className="p-5 elevation-1">
            ...
            {locked && visibleTeams.length === 0 ? (
              ...
              <p className="text-sm text-muted-foreground italic">
                Primero completa "{prevCat!.label}" ({prevSelectedIds!.length}/{prevCat!.count}) para habilitar esta fase.
              </p>
            ) : (
              (() => {
